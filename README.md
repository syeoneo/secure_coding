...
### 1. 저장소 복제 및 프로젝트 폴더 이동

```powershell
git clone https://github.com/syeoneo/secure_coding.git
cd secure_coding
```

ZIP으로 내려받은 경우 압축을 푼 뒤 `README.md`와 `app.py`가 있는 프로젝트 루트 폴더에서 아래 명령을 실행합니다.
...
서버를 처음 실행하면 프로젝트 루트에 `market.db`가 자동 생성되고 필요한 테이블과 인덱스가 초기화됩니다. `market.db`는 실행 데이터이므로 GitHub에는 포함하지 않습니다.

테스트를 위해 일반 사용자 계정 2개와 관리자 계정 1개를 생성하는 것을 권장합니다. 일반 계정은 웹 회원가입 화면에서 만들고, 관리자 권한은 아래의 `make_admin.py`로 부여합니다.
...
과제에서 제공된 보안 항목과 프로젝트 적용 여부는 다음 문서에서 확인할 수 있습니다.

- [`secure_coding_checklist.csv`](secure_coding_checklist.csv): 제공 보안 체크리스트
- [`GUIDELINE_AUDIT.md`](GUIDELINE_AUDIT.md): 기능·보안·제출 요구사항 매핑
- [`docs/FINAL_MANUAL_TEST_CHECKLIST.md`](docs/FINAL_MANUAL_TEST_CHECKLIST.md): 수동 테스트 절차
...
4. .env 생성 및 SECRET_KEY 설정
5. app.py 실행
6. 회원가입
7. 필요 시 make_admin.py로 관리자 권한 부여
8. verify_project.py와 수동 체크리스트 실행
```
